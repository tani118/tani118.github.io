        ...
        <div className="max-w-4xl mx-auto px-8 py-12">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              <button
                onClick={onClose}
                className={`font-mono text-lg transition-colors duration-300 hover:underline ${
              ...
              {!showBackToAllBlogs && onBackToAllBlogs && (
                <button
                  ...
                  className={`font-mono text-sm px-3 py-1 border transition-all duration-300 ${
                    isHovered 
          ...
          {/* Blog content */}
          <article className={`markdown-content prose prose-lg max-w-none transition-colors duration-300 font-mono ${
            isHovered 
            ...
            <h1 className={`font-mono text-4xl font-bold mb-8 transition-colors duration-300 ${
            ...
            <div className={`leading-relaxed transition-colors duration-300 ${
              ...
              components={{
                ...
                h1: ({children}) => (
                  <h1 className={`font-mono text-3xl font-bold mb-6 mt-8 transition-colors duration-300 ${
                    isHovered ? 'text-black' : 'text-white'
                  }`}>{children}</h1>
                ),
                h2: ({children}) => (
                  <h2 className={`font-mono text-2xl font-bold mb-4 mt-8 transition-colors duration-300 ${
                    isHovered ? 'text-black' : 'text-white'
                  }`}>{children}</h2>
                ),
                h3: ({children}) => (
                  <h3 className={`font-mono text-xl font-bold mb-4 mt-6 transition-colors duration-300 ${
                    isHovered ? 'text-black' : 'text-white'
                  }`}>{children}</h3>
                ),
                p: ({children}) => (
                  <p className={`font-mono mb-4 leading-relaxed transition-colors duration-300 ${
                    isHovered ? 'text-gray-800' : 'text-gray-300'
                  }`}>{children}</p>
                ),
                code: ({inline, className, children, ...props}) => {
                  if (inline) {
                    return (
                      <code 
                        className={`px-2 py-1 rounded font-mono text-sm transition-colors duration-300 ${
                          isHovered 
                ...
                pre: ({children}) => {
                  ...
                  return (
                    <div className="relative group">
                      <pre className={`p-4 rounded-lg overflow-x-auto font-mono text-sm my-6 transition-colors duration-300 border ${
                        isHovered 
                ...
                img: ({src, alt, title}) => (
                  <div className="my-8">
                    ...
                    />
                    {alt && (
                      <p className={`text-center mt-2 text-sm font-mono transition-colors duration-300 ${
                ...
                th: ({children}) => (
                  <th className={`border px-4 py-3 font-mono font-bold text-left transition-colors duration-300 ${
                    isHovered 
                ...
                td: ({children}) => (
                  <td className={`border px-4 py-3 font-mono transition-colors duration-300 ${
                    isHovered 
